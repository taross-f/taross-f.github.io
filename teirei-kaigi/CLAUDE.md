...
- 毎日の日次定例会議(Zoom風UI)に出席する。月〜金の5日制(通常モード `mode === "campaign"`)。
...
6. ~~正解時に異変名を表示している(transition.detail)~~ → 伏せるよう変更済み(図鑑でのみ開示)
7. 音による異変(`sound.js` の合成エンジンを活かす。現状は異変が視覚に偏っている)
8. デイリーチャレンジ(日付シードで全員同条件 → シェア導線)
